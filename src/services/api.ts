...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    const shouldRetry =
      isRetryable && config && !config._retry && config.method === "get";
    ...
    if (error.message === "Network Error") {
      console.warn(
        "API Network Error - may be CORS, blocked request, or server restart",
        {
...
export const authService = {
  ...
  adminLogin: (data: { id: string; password: string }) =>
  ...
  acknowledgeCommission: () => api.post("/auth/acknowledge-commission"),
...
export const orderService = {
  ...
  getStats: (sellerId?: string) =>
    api.get("/orders/stats", { params: { sellerId } }),
...
export const settingsService = {
  ...
  toggleSellerTrial: (
    sellerId: string,
    data: { isTrialActive: boolean; trialDurationDays?: number },
