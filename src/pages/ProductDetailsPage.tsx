  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setRelatedProducts(
          allProductsRes.data.filter((p: Product) => p._id !== id).slice(0, 4),
        );

        // Fetch seller details and product count
        const sellerId =
          typeof productData.sellerId === "object"
            ? productData.sellerId._id
            : productData.sellerId;
        if (sellerId) {
          try {
            const sellerProductsRes =
  ...
  return (
    <div className="pb-20">
      ...
      <div className="container mx-auto px-4">
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star
                    key={s}
                    className={`w-4 h-4 ${s <= Math.round(product.averageRating || 0) ? "fill-amber-400 text-amber-400" : "text-slate-200"}`}
                  />
                ))}
              ...
              {product.sellerId?.isLocked ? (
                <div className="p-6 bg-rose-50 border border-rose-100 rounded-3xl text-center">
                  ...
                  <p className="text-secondary text-sm font-bold">
                    عذراً، هذا البائع غير متاح حالياً لاستقبال الطلبات بسبب
                    أعمال صيانة في الحساب.
                  </p>
                </div>
              ) : (
                ...
                <div>
                  <div className="text-sm font-bold text-slate-900">
                    {product.deliveryAvailable
                      ? `توصيل متاح (${product.deliveryFee} ج.م)`
                      : "توصيل غير متاح"}
                  </div>
                  <div className="text-[10px] text-slate-500">
                    {product.deliveryAvailable
                      ? "توصيل سريع لباب المنزل"
                      : "يرجى التواصل مع البائع"}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
        ...
        {/* Seller Info Section */}
        <div id="seller-details-section" className="mb-20 px-4">
          <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/40 max-w-[1020px] mx-auto overflow-hidden">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
              <div className="flex flex-col items-center lg:items-start gap-8 flex-1 text-center lg:text-right w-full">
                <div className="flex flex-col md:flex-row items-center gap-8 w-full">
                  <div className="relative shrink-0">
                    <div className="w-28 h-28 md:w-36 md:h-36 bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] flex items-center justify-center font-black text-5xl md:text-6xl text-primary border border-slate-100 shadow-inner overflow-hidden">
                      {typeof product.sellerId === "object"
                        ? product.sellerId.name?.charAt(0) || "S"
                        : "S"}
                    </div>
                    <div className="absolute -bottom-1 -right-1 md:-bottom-2 md:-right-2 w-10 h-10 md:w-12 md:h-12 bg-emerald-500 text-white rounded-2xl flex items-center justify-center shadow-lg border-4 border-white">
                  ...
                  <div className="flex-1 w-full">
                    <div className="flex flex-col md:flex-row items-center gap-4 mb-4 justify-center md:justify-start">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="px-4 py-2 bg-emerald-50 text-emerald-600 text-xs font-black rounded-full uppercase tracking-widest border border-emerald-100">
                          موثوق
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 md:gap-6 mb-8">
                      <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-100">
                        <MapPin className="w-4.5 h-4.5 text-primary" />
                        <span className="font-bold">
                          {typeof product.sellerId === "object"
                            ? product.sellerId.location?.address ||
                              "القاهرة، مصر"
                            : "مصر"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-100">
                        ...
                        <span dir="ltr" className="font-bold">
                          {typeof product.sellerId === "object"
                            ? formatDisplayPhone(product.sellerId.phone)
                            : ""}
                        </span>
                      </div>
                    </div>

                    <div className="grid grid-cols-3 gap-3 md:gap-5 max-w-sm md:max-w-md mx-auto md:mx-0">
                      <div className="bg-slate-50/80 px-2 py-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] text-center border border-slate-100 transition-all hover:border-amber-200">
                        <div className="flex items-center justify-center gap-1.5 text-amber-500 mb-1.5">
                        ...
                        <div className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-tighter">
                          التقييم
                        </div>
                      </div>
                      <div className="bg-slate-50/80 px-2 py-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] text-center border border-slate-100 transition-all hover:border-primary/30">
                        <div className="text-sm md:text-lg font-black text-slate-900 mb-1.5 leading-none">
                      ...
                      <div className="bg-slate-50/80 px-2 py-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] text-center border border-slate-100 transition-all hover:border-emerald-200">
                        <div className="flex items-center justify-center gap-1.5 text-emerald-500 mb-1.5">
              ...
              <div className="flex flex-col sm:flex-row lg:flex-col gap-4 w-full lg:w-80 shrink-0">
                {product.sellerId?.isLocked ? (
                  <div className="bg-rose-50 text-rose-600 px-10 py-6 rounded-[2rem] text-sm font-black text-center border border-rose-100 w-full shadow-lg shadow-rose-500/5">
              ...
              {user ? (
                ...
                <div className="text-center py-4">
                  <p className="text-sm text-white/60 mb-4">
                    يرجى تسجيل الدخول لتتمكن من تقييم المنتج
                  </p>
                  <Link
                    to="/login"
                    className="inline-block bg-white text-slate-900 px-6 py-2 rounded-xl font-bold text-sm hover:bg-slate-100 transition-all"
                  >
                    تسجيل الدخول
                  </Link>
                </div>
