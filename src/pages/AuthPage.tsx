...
import { Mail, Lock, User, Phone, MapPin, ShieldCheck, Map as MapIcon } from 'lucide-react';
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-secondary text-sm">
            {type === 'login' ? 'سجل دخولك للوصول إلى حسابك' : 'انضم إلينا وابدأ التسوق اليوم'}
          </p>
        </div>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                ...
                {showMap && (
                  <div className="h-48 w-full rounded-2xl overflow-hidden border border-slate-200 shadow-inner">
                    <MapContainer center={position} zoom={13} style={{ height: '100%', width: '100%' }}>
                      ...
                      />
                      <LocationPicker position={position} setPosition={setMapPosition} setAddress={setAddress} />
                    </MapContainer>
                  </div>
                )}
              </div>
              <div className="relative">
                <ShieldCheck className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-secondary" />
                <select 
                  value={formData.role}
                  onChange={(e) => setFormData({ ...formData, role: e.target.value as any })}
                  className="w-full bg-slate-50 border-none rounded-2xl pr-12 pl-4 py-4 text-sm focus:ring-2 focus:ring-primary/20 appearance-none"
                >
          ...
          >
            {loading ? 'جاري التحميل...' : (type === 'login' ? 'تسجيل الدخول' : 'إنشاء الحساب')}
          </button>
