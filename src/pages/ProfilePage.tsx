...
export const ProfilePage = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "info" | "security" | "orders" | "earnings" | "wishlist" | "notifications"
  >("info");
  ...
  const [position, setPosition] = useState<[number, number]>([
    user?.location?.lat || 24.7136,
    user?.location?.lng || 46.6753,
  ]);
      ...
      if (activeTab === "notifications" && user) {
        try {
          ...
          if (err.message === "Network Error" && retryCount < 3) {
            console.warn(
              `Retrying profile notifications fetch (${retryCount + 1})...`,
            );
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <div className="flex items-center gap-4 mb-12">
        <Link
          to="/"
          className="w-10 h-10 border border-slate-200 rounded-full flex items-center justify-center hover:bg-slate-50 transition-colors"
        >
        ...
        <div className="lg:col-span-1 flex flex-col items-center gap-8">
          ...
          <div className="w-full flex flex-col gap-2">
            ...
            >
              ...
              {user.role === "seller" ? "طلبات البيع" : "طلباتي"}
            </button>
            <button
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50">
                  ...
                  >
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-secondary px-2">
                        الاسم الكامل
                      </label>
            ...
            {activeTab === "orders" && (
              ...
              >
                          ...
                          <tbody>
                            {orders
                              ...
                              .map((order) => (
                                ...
                                >
                                  <td className="py-4 pr-4 font-bold text-xs">
                                    {order._id.slice(-6).toUpperCase()}
                                  </td>
                                  <td className="py-4 text-sm font-bold text-slate-900">
                                    {order.productId?.name || "منتج غير متوفر"}
                                  </td>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50">
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="text-lg font-black text-slate-900">
                      سجل الطلبات (المكتملة والمرفوضة والملغاة)
                    </h3>
                  </div>
              ...
              >
                ...
                {notificationsLoading ? (
                  ...
                  <div className="bg-white p-12 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 text-center">
                    ...
                    <p className="text-secondary text-sm">
                      ستظهر التنبيهات هنا عندما تتوفر عروض جديدة أو يتوفر منتج
                      في المخزون
                    </p>
                  </div>
                ) : (
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          notification.read
                            ...
                            : "border-blue-100 shadow-lg shadow-blue-500/5 ring-1 ring-blue-50",
                        )}
                      >
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/50">
                      ...
                      {earnings?.totalCommission > 0 && (
                        <div className="text-[10px] text-slate-400 mt-1">
                          خصم{" "}
                          {(earnings?.globalCommissionRate * 100 || 10).toFixed(
                            0,
                          )}
                          % عمولة (ج.م {earnings?.totalCommission || 0})
                        </div>
                      )}
                    </div>
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/50">
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/50">
                      ...
                      <div className="text-2xl font-black text-slate-900">
                        {earnings?.rating && earnings.rating > 0
                          ? earnings.rating.toFixed(1)
                          : "جديد"}
                      </div>
                      <div className="text-xs font-bold text-secondary">
                        تقييم المتجر{" "}
                        {earnings?.numReviews > 0 && `(${earnings.numReviews})`}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-xs font-bold text-secondary">
                            عمولة المنصة (
                            {(
                              earnings?.globalCommissionRate * 100 || 10
                            ).toFixed(0)}
                            %)
                          </div>
                          <div className="text-2xl font-black text-primary">
                            ج.م {earnings?.commission || 0}
                          </div>
                        </div>
