  ...
  async (req, res) => {
    try {
      ...
      const followers = await User.find({
        followingSellers: (req as any).user.id,
      });
...
router.get("/seller/:sellerId", async (req, res) => {
  try {
    ...
    const [products, seller] = await Promise.all([
      ...
      User.findById(sellerId).select("name email phone location isLocked"),
    ]);
    ...
    products.forEach((p) => {
      if (p.numReviews && p.numReviews > 0) {
        sumRating += (p.averageRating || 0) * p.numReviews;
        totalReviews += p.numReviews;
      }
    });
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Product.findById(req.params.id).populate(
      "sellerId",
      "name email phone location isLocked",
    );
  ...
  try {
    ...
    if (updateData.images && Array.isArray(updateData.images)) {
      ...
      const removedImages = product.images.filter(
        (img) => !updateData.images.includes(img),
      );
