...
router.put("/", authMiddleware, roleMiddleware(["admin"]), async (req, res) => {
  try {
    const { globalCommissionRate, isTrialEnabled, trialDurationDays } =
      req.body;

    // Validation: Commission rate must be between 0 and 25% (0.25)
    if (globalCommissionRate !== undefined) {
      if (
        typeof globalCommissionRate !== "number" ||
        globalCommissionRate < 0 ||
        globalCommissionRate > 0.25
      ) {
        return res
          .status(400)
          .json({ message: "العمولة يجب أن تكون بين 0% و 25% فقط" });
      }
    }
    ...
    if (globalCommissionRate !== undefined) {
      settings.globalCommissionRate = globalCommissionRate;
      settings.commissionUpdateDate = new Date();
    }
    if (isTrialEnabled !== undefined) settings.isTrialEnabled = isTrialEnabled;
    if (trialDurationDays !== undefined)
      settings.trialDurationDays = trialDurationDays;
