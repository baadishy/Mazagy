...
router.get("/public-stats", async (req, res) => {
  try {
    const [totalOrders, sellersCount, buyersCount, totalUsers, settings] =
      await Promise.all([
    ...
    res.json({
      ...
      buyersCount,
      totalUsers,
      commissionRate: settings?.globalCommissionRate || 0.1,
  ...
  try {
    ...
    if (!/^01[0125][0-9]{8}$/.test(cleanPhone)) {
      return res
        ...
        .json({
          message:
            "تنسيق رقم الهاتف غير صحيح. يجب أن يكون رقماً مصرياً صالحاً يبدأ بـ 01",
        });
    }
  ...
  try {
    ...
    if (currentStatus === "pending") {
      if (status !== "confirmed" && status !== "rejected") {
        return res
          .status(400)
          .json({ message: "انتقال غير صالح للحالة من قيد الانتظار" });
      }
      ...
      if (status !== "delivered") {
        return res
          ...
          .json({
            message:
              "لا يمكن رفض الطلب بعد تأكيده. يجب التواصل مع الإدارة في حال وجود مشكلة.",
          });
      }
    }
    ...
    if (status === "confirmed" || status === "delivered") {
      ...
      if (
        ...
        const sellerEarning =
          productTotal - commissionAmount + (order.deliveryFee || 0);
...
router.get(
  "/stats",
  authMiddleware,
  roleMiddleware(["seller", "admin", "moderator", "user"]),
  async (req, res) => {
    try {
      const user = (req as any).user;
      let query: any = {};
      ...
      const totalProductEarnings = relevantOrders.reduce(
        (sum, order) => sum + (order.price || 0) * (order.quantity || 0),
        0,
      );
      const totalDeliveryEarnings = relevantOrders.reduce(
        (sum, order) => sum + (order.deliveryFee || 0),
        0,
      );

      const totalGMV = totalProductEarnings + totalDeliveryEarnings;
      const totalPlatformCommission = relevantOrders.reduce(
      ...
      if (user.role === "admin") {
        ...
        suspiciousSellers = suspiciousData
          .map((item) => {
            let reasons = [];
            if (item.rejected / item.total > 0.3)
              reasons.push("معدل رفض عالي (>30%)");
            if (item.confirmed > 10)
              reasons.push("عدد كبير من الطلبات المؤكدة غير المكتملة");
      ...
      relevantOrders.forEach((o) => {
        ...
        const amount =
          user.role === "admin" && !req.query.sellerId
            ? o.commissionAmount || 0
            : o.sellerEarning ||
              (o.price || 0) * (o.quantity || 0) +
                (o.deliveryFee || 0) -
                (o.commissionAmount || 0);
      ...
      // Calculate seller rating and numReviews from their products
      let averageRating = 0;
      let numReviews = 0;
      if (sellerProducts.length > 0) {
        const ratingInfo = await Product.aggregate([
        ...
        if (ratingInfo.length > 0) {
          averageRating = ratingInfo[0].avgRating || 0;
          numReviews = ratingInfo[0].totalReviews || 0;
        }
      }
      ...
      const stats = {
        ...
        totalSellers:
          user.role === "admin"
            ? await User.countDocuments({ role: "seller" })
            : 0,
  ...
  async (req, res) => {
    try {
      ...
      const sellersWithStats = sellers.map((seller) => {
        const stats = statsMap[seller._id.toString()] || {
          totalSales: 0,
          commission: 0,
        };
        return {
