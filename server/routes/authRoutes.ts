...
const getCommissionInfo = async (user: any) => {
  const settings = await Settings.findOne();
  const defaultRate = 0.1;
  const currentRate = settings ? settings.globalCommissionRate : defaultRate;

  if (user.role !== "seller") {
    return { hasUnacknowledgedCommission: false, commissionRate: currentRate };
  }

  // If settings don't exist yet, or have never been updated (epoch 0), don't show modal
  if (
    !settings ||
    !settings.commissionUpdateDate ||
    new Date(settings.commissionUpdateDate).getTime() === 0
  ) {
    return { hasUnacknowledgedCommission: false, commissionRate: currentRate };
  }

  const updateTime = new Date(settings.commissionUpdateDate).getTime();
  const ackTime = user.lastCommissionAckDate
    ? new Date(user.lastCommissionAckDate).getTime()
    : 0;

  // IMPORTANT: Only show if updateTime is strictly GREATER than acknowledgment time.
  // We use a 2-second buffer to handle any millisecond drift during DB saves.
  const hasUnacknowledgedCommission = updateTime > ackTime + 2000;

  return {
    hasUnacknowledgedCommission,
...
router.post("/signup", async (req, res) => {
  try {
    ...
    const commissionInfo = await getCommissionInfo(user);
...
// Acknowledge Commission Change
router.post("/acknowledge-commission", authMiddleware, async (req, res) => {
  try {
    const user = await User.findById((req as any).user.id);
    if (!user) return res.status(404).json({ message: "User not found" });

    user.lastCommissionAckDate = new Date();
    await user.save();

    res.json({
      success: true,
      lastCommissionAckDate: user.lastCommissionAckDate,
    });
  } catch (error) {
    res.status(500).json({ message: "Server error" });
  }
...
router.get("/profile", authMiddleware, async (req, res) => {
  try {
    let user: any = await User.findById((req as any).user.id).select(
      "-password",
    );
    if (!user) return res.status(404).json({ message: "User not found" });
